...
)
{
	...
	while (true) {
		...
		if (strcmp(incomingString, "!quit") == 0) {
			cout << "done with another client!\n";
			return 0;
		}

		int length = checkLength(incomingString, sizeof(incomingString));
		
		send(sock, reinterpret_cast<char*>(&length), sizeof(int), NULL);
		
		if (length > 15) {
			char outcomingString[64];
			for (int i = 0; i < sizeof(outcomingString); i++) {
				if (incomingString[i] == ' ') {
					memcpy(outcomingString, incomingString, i);
					outcomingString[i] = '\0';
